...
module.exports = {
  createUser: (req, res) => {
    ...
    connection.query(
      ...
      (err, results) => {
        if (err) {
          return res.status(err).json({ msg: "database connection err-1" });
        }
        if (results.length > 0) {
          return res
            .status(400)
          ...
          register(req.body, (err, results) => {
            if (err) {
              console.log(err);
              return res.status(500).json({ msg: "database connection err0" });
            }
            ...
            connection.query(
              ...
              (err, results) => {
                if (err) {
                  return res
                    .status(err)
                    .json({ msg: "database connection err1" });
                }
                ...
                profile(req.body, (err, results) => {
                  if (err) {
                    ...
                    return res
                      .status(500)
                      .json({ msg: "database connection err2" });
                  }
  ...
  getUserById: (req, res) => {
    ...
    userById(req.id, (err, results) => {
      if (err) {
        console.log(err);
        return res.status(500).json({ msg: "database connection err3" });
      }
      if (!results) {
  ...
  login: (req, res) => {
    ...
    getUserByEmail(email, (err, results) => {
      if (err) {
        console.log(err);
        res.status(500).json({ msg: "database connection err4" });
      }
      if (!results) {
        return res
          .status(404)
